...
flowchart LR
    ...
    subgraph S1["① Define Spec"]
        direction TB
        A1["SRE defines:<br/>SLO / Dependencies<br/>Fault modes / Constraints"]
        M1{{"Realizability<br/>Check"}}
        R1["Can these specs be<br/>satisfied simultaneously?<br/><br/>YES → proceed to ②<br/>NO → relax & retry"]
        A1 --> M1 --> R1
    end
    ...
    subgraph S2["② Build Controller"]
        ...
        M2{{"Reactive<br/>Synthesis"}}
        R2["Auto-generate a strategy<br/>guaranteed correct<br/><br/>Output: correct-by-construction<br/>state machine"]
        A2 --> M2 --> R2
    end
    ...
    subgraph S3["③ Runtime Ops"]
        ...
        M3{{"Bounded Model<br/>Checking"}}
        R3["Will this action violate<br/>any invariant in k steps?<br/><br/>APPROVE → execute<br/>REJECT → block + trace"]
        A3 --> M3 --> R3
    end
    ...
    subgraph S4["④ Post-Incident ⟳①"]
        ...
        M4{{"BMC Reverse +<br/>Counterexample"}}
        R4["How did system reach<br/>fault state step by step?<br/><br/>🔄 Output feeds back to ①:<br/>new specs + fault modes"]
        A4 --> M4 --> R4
    end
...
| Phase | Key Activity | Formal Method | Question Answered |
|---|---|---|---|
| ① Define Spec | SRE writes SLO + constraints | Realizability Check | Can these specs be satisfied? |
| ② Build Controller | Generate strategy from specs | Reactive Synthesis | Can we auto-generate a correct strategy without writing SOPs? |
...
flowchart LR
    ...
    subgraph S4["④ 事后复盘 ⟳①"]
        ...
        M4{{"BMC 逆向 +<br/>反例分析"}}
        R4["系统怎么一步步走到故障态?<br/>哪条路径可以被堵住?<br/><br/>🔄 输出回到①:<br/>补充规约 + 新故障模式"]
        A4 --> M4 --> R4
    end
